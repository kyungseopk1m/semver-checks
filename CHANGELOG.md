...
This release is about a class of false positive nobody had seen, because the corpus does not contain it. Scanning the newest adjacent release of eighteen packages, looking for an under-bump worth reporting to its author, turned up `jose` 6.2.10: a patch release, three days old, with 42 proven breaks against a consumer that compiles on both sides. `SignJWT` had moved to extending a base whose instance type is the interface it used to implement, and every member it inherits was read as removed.

The class rules never received the base lookups the interface rules have used since they landed. A member hoisted onto a base is a refactor no consumer can see, and the tool called it a confident break. The mixin spelling is not what broke it: a plain `extends` broke the same way, and the corpus missed both because none of its pairs happened to move a class member.

The rest is the MCP surface again, this time about what an agent can act on rather than what it can reach.

### Accuracy

`--strict` fires on 36 of 111 pairs, 35 of them real: precision 97.2%, recall 81.4%. `--strict-review` fires on all 43 real breaks and on 10 pairs that hold none. Neither figure moves in this release, and neither should: the corpus holds no pair with a hoisted class member, which is the whole point of the entry above.

The 8 pairs `--strict` stays quiet on, each attributed to why:

| Pair                        | Why it stays quiet                                                                                                                                               |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hono` 4.12.19 -> 4.12.20   | The guard that refuses to call two unresolved stand-ins unrelated. Pinned by `scope-unexported-internal-swapped`                                                 |
| `ky` 1.14.1 -> 1.14.2       | Same guard                                                                                                                                                       |
| `bullmq` 5.80.11 -> 5.80.12 | The `any` guard from 0.11.0: one member typed `any` makes the two texts mutually assignable                                                                      |
| `ts-pattern` 5.6.0 -> 5.6.1 | The declaration guard from 0.11.0, plus `generic-constraint-changed` being review-only on purpose                                                                |
| `got` 14.6.4 -> 14.6.5      | `interface-call-signature-changed` is review-only on purpose; separating a leading overload from an optional parameter needs signature-level variance            |
| `valibot` 1.3.1 -> 1.4.0    | Not a miss. A parameter that accepts more is a minor, and the break is visible only to a consumer naming `Parameters<typeof f>[0]`                               |
| `hono` 4.12.28 -> 4.12.29   | The probe declines to guess, since the verdict would rest on two stand-ins being unrelated                                                                       |
| `hono` 4.12.18 -> 4.12.19   | Not attributable. The change that breaks a consumer is reported nowhere, and the findings that do fire are on other symbols. This is the one extraction gap left |

Two of the 43 that `--strict-review` catches are a coincidence rather than a catch: on `hono` 4.12.18 -> 4.12.19 and `ky` 1.14.1 -> 1.14.2 it fires on a symbol other than the one that breaks. Counting only the pairs where the finding is the break puts it at 41 of 43.

### Breaking

- **A source that is empty or malformed is refused, on both surfaces.** An empty source fell through to the path branch, where an empty path resolves to the working directory, so `compare "" <new>` analyzed the current tree against itself and exited 0 reporting no changes. A gate that answers green to a question nobody asked is worse than one that answers wrong, and this is the shape that produced it. Anything that reaches `resolveSourceInput` is covered, so the CLI, the library and the MCP server all refuse it now.
- **Arguments outside a tool's schema are refused rather than dropped.** `additionalProperties: false` is advertised on both MCP tools, but the SDK hands `tools/call` arguments through as they arrive, so a caller reaching for a CLI flag name got an analysis that quietly ignored it and a confident answer to a question they did not ask. The refusal names the argument and lists the accepted ones. An LLM caller is the case this matters for, since it cannot re-read its own request to notice what went missing.
- **MCP error text carries the code inside the `Error:` prefix.** It reads `Error: [invalid_argument] message`. The prefix is unchanged, so a consumer matching on it still matches, but one taking the message as everything past a fixed offset needs the new `error` field or a parse.

### Fixed

- **A class member hoisted onto a base is no longer read as removed.** Inherited members are deliberately not flattened into a class symbol, so resolving the bases is what tells a move from a real removal, and the class path was never given the lookups. `jose` 6.2.10 is the case that found it, at 42 proven breaks in a patch release; two fixtures pin the mixin form and the plain form, since only the first was seen in the wild and both were broken.
- **A removal on a class whose base the snapshot does not hold stays a MAJOR but is no longer proven.** A subpath entry is often one symbol wide, so `jose`'s `./jwt/sign` exports `SignJWT` while the base it names lives in `./types`, and from there the members a base carries are not visible at all. Reporting them as removed turns not having looked into having left, which is the one thing a proven grade must never do. Additions are untouched, since an addition is not a claim about where something went.
- **A probe whose generic is named after a scope stub answers again.** The stubs sit at file scope and so do the synthesized type parameter aliases, so a collision was a duplicate identifier rather than a shadowing and the whole probe errored out. That left a comparison worse off for having a scope installed than for having none. Only the colliding stub is withheld, and only for that probe.
- **`--install-deps` does what it says.** citty put the flag under its literal kebab key while the camelCase default kept winning the lookup, so it parsed without complaint and then did nothing. Both commands declare the alias now. Every place the flag is documented uses the kebab spelling.
- **`oldAs`, `newAs` and `pathAs` accept `ref`, and read an empty string as unset.** `ref` is the CLI spelling and the one the monorepo advice uses, so a caller who read either landed on a refusal. An empty kind is how a template that leaves an input blank spells not specified, which is how the CLI has always read it.
- **A malformed source and a missing one are told apart.** Both were reported the same way, though only the first is fixed by sending different arguments.

### Added

- **Every MCP report carries a `gate` object** saying what each CLI gate would do with it: `{ "strict": "pass" | "fail", "strictReview": "pass" | "fail" }`. That is what the exit code carries on the command line, and over MCP a caller would otherwise rebuild a two-branch rule out of two counts and a verdict field. The rule now lives in one place both surfaces read, so they cannot drift.
- **MCP failures carry a code**, in the message text and in an `error` field: `invalid_argument` for something a different call would fix, `analysis_failed` for a call that was well-formed and could not be answered. Retrying the second unchanged is wasted work, and prose was the only thing distinguishing them.
- **The GitHub Action takes `old-as`, `new-as` and `install-deps`.** The CLI and the MCP server had all three. The README tells anyone with a monorepo tag shaped like a package spec to force git resolution, and an action user could not.
- **The `semver_compare` description says what the two gates promise**, since a passing `strict` is not a clean bill of health and that distinction lived only in the README, where nothing reading the schema will find it.
...
- **An `exports` subpath that names only its runtime file now resolves to the declaration beside it.** TypeScript substitutes the analogous extension (`./x.js` → `./x.d.ts`) before giving up, and without it a subpath shipping no `types` condition read as having no public surface. Six of `kysely`'s subpaths and two of `highlight.js`'s were skipped for this reason; `kysely`'s extracted surface goes from 3 symbols to 599. Substituted candidates rank below every declared types path all the way through resolution, so a package that says where its types are keeps the entry it resolves to today, and substitution is offered only to a published tarball so a working tree is still analyzed from its source rather than from a stale `dist/`. A subpath whose sibling declaration does not exist is still skipped, which is the right answer for packages like `axios` that genuinely ship no per-file declarations. Across the gate packages this changes the resolved entry for `kysely` and adds two thin subpath entries for `highlight.js`; every other package resolves byte-identically, and only `kysely`'s gate verdicts move.

### Changed

- **`--strict` grades six more rules as `proven`, and recall goes from 37.8% to 67.4%.** `interface-method-removed`, `required-interface-method-added` and `required-property-added` join the kind allow-list, restoring the symmetry the file already argued for: `property-removed` and `class-method-removed` were proven while their interface counterparts were not, and an interface exists to be implemented as well as consumed. `interface-heritage-changed` is proven when a base disappeared, which takes its inherited members with it; a base that was only added is review-only, because whether it obliges anyone turns on members this clause cannot see. `property-type-changed`, `class-property-type-changed` and `type-alias-changed` are proven when the variance probe reaches a verdict: an invariant position has both a reader and a writer, so a resolved non-equivalence breaks one of them. A probe that bailed still grades review-only, since a bail is evidence of nothing. Precision over the corpus goes from 93.3% to 96.7% at the same time, because most of what was promoted was already correct and merely ungraded.
- **The accuracy gate corpus grows to 111 adjacent release pairs across 24 packages**, from 75 across 17. `kysely`, `jose`, `ioredis`, `dayjs` and `highlight.js` fill surface shapes the corpus had no coverage of: subpath entrypoints, `export =` over a namespace, a client class with eight constructor overloads, and a surface declared entirely inside `declare module`. `clsx` and `p-limit` were added because this file and the test suite both cited them as archetypal false positives, and compiling them proved otherwise. 37 pairs carry probe programs alongside the package's main consumer.
- **A finding recorded as a deliberate over-report is a hypothesis until it is compiled, and eight of them were wrong.** `hono` 4.12.29 to 4.12.30 was documented as safe because a narrowed parameter is contravariant, and the probe that existed tested only the implementer side; a caller relaying a Node `Buffer` breaks. `jose` 6.2.4 to 6.2.5, `kysely` 0.29.2 to 0.29.3, `lru-cache` 11.5.0 to 11.5.1, `highlight.js` 11.9.0 to 11.10.0, `ioredis` 5.8.1 to 5.8.2 and `axios` 1.18.1 to 1.19.0 were the same story. So was `clsx`: widening an input union is safe only while the alias stays inside the library, and a consumer that re-declares the accepted union in its own public prop type stops compiling when it gains a member. Only the `zod` `version` constant remains classified as an over-report, and its note is corrected: it breaks anyone who writes the literal down, comparison included, not only someone who pinned it.
- **The variance probe reads the package's own declarations, and recall goes from 67.4% to 79.1%.** A serialized type text is printed by the checker, so it names the types the package declares about itself; the probe resolved those names in a program holding nothing but the ES libs, so `ClassArray | ClassDictionary`, `P.Pattern<T> & UnknownProperties` and `core.$ZodTypeDiscriminable<Disc>` all came back undecidable and the findings stayed review-only. Both snapshots are now rendered back into ambient declarations and installed into that program, the old side under one namespace and the new side under another, so each text resolves against its own version. Enums and classes are deliberately left out: they are nominal, so the same unchanged enum would appear as two unrelated types and every text mentioning it would probe as an unrelated change. Precision goes from 96.7% to 97.1% at the same time, and the naive baseline no longer catches a single break the type analysis misses.
- **A finding called breaking is a hypothesis too, and eleven shapes proved it.** Every promotion was checked against the corpus, which agreed, and then checked again by constructing the release that ought to have been harmless and compiling it. Renaming a parameter in a call signature, adding an optional trailing parameter to one, moving a member between an interface and one of its bases in either direction, inlining a base into the interface that extended it, dropping a base that declared nothing, dropping a base whose members the interface now declares itself, and adding a base whose members are all optional all break no consumer, and all failed the gate. `interface-call-signature-changed` goes back to review-only, which costs `got` 14.6.4 to 14.6.5, where a leading overload was added: separating that from an added optional parameter takes signature-level variance the text-multiset comparison does not do. The heritage clause and the member rules resolve the bases now, so a member still reachable through one is not reported at all, and a dropped base is proven only when something it carried has left the surface. A base still listed under the same name with different type arguments is not a drop and stays proven, since re-parameterizing moves the types of everything it contributes. Fixtures pin every shape, because the corpus holds none of them and its numbers did not move when they were fixed.
- **The variance probe no longer answers about a DOM global a package shadows.** It resolves a type by name in a project of its own, which carried the DOM lib, so a package declaring its own `Element`, `Node`, `Response` or `Event` got a confident verdict about a type it never mentions. The probe carries only the ES libs now; an unresolvable name reads as undecidable, which is what keeps a name it cannot see out of `--strict`.
- **The naive baseline no longer keeps pace.** The 45-line exported-name-and-arity control scores 87.5% / 32.6% against the tool's 97.2% / 81.4%. They agree on 14 breaks, 21 belong to the type analysis alone, none to the baseline alone, and 8 defeat both. The previous revision of this entry recorded a tie on recall; grading is what broke it, and the probe scope is what took the baseline's last exclusive catch (`ky` 1.11.0 to 1.12.0) away from it.
- **A name no declaration covers gets an opaque stand-in, and a verdict resting on two different stand-ins is thrown away.** Type texts routinely name types a package does not export — `ts-pattern` reaches for `SelectionType`, `MergeGuards` and eight more from an entry point that declares none of them — and dropping every declaration that mentions one took that entry from 35 declarations to 3. A stand-in is shared by both sides, so the same spelling means the same type, which is the model the whole tool already runs on. What it must not do is decide anything: two different unresolved names probe as unrelated by construction, so a package that renamed an internal shape no consumer can name would fail the gate. When the two texts name different stand-ins the relation goes back to undecidable. Reserved type names (`bigint`, `object`, `symbol`) are never stood in for, because `type bigint = ...` is an error no repair round can clear and three of them cost `valibot` all 449 of its declarations; neither is a name the snapshot holds but the scope leaves out, because a stand-in carries a brand property and an interface extending an omitted class would inherit a member the class never had.
- **A probe that answers "equivalent" across a change of name is reported review-only rather than dropped.** Mutual assignability is not identity: TypeScript ignores `readonly` on a property, compares a method bivariantly where it compares a function-typed property strictly, drops `this` parameters, and applies the excess-property check only to a fresh object literal. Before the probe could resolve package-local names these pairs bailed and were reported for want of an answer; resolving them turned four demonstrated breaks into silence, none of which the corpus contains. A pure textual rewrite names the same declarations on both sides and is still a no-op, and no `proven` verdict moves in either direction, so `--strict` is unaffected and the corpus scores identically.
- **A rendered declaration that mentions `any` is not installed into the probe.** `mentionsAny` bails the probe when either compared text says the word, but a declaration the two texts _resolve through_ can say it instead, and one member typed `any` makes its whole containing type bidirectionally assignable: `Sink` then compares equivalent to `Sink & { write(chunk: Uint8Array): void }` and the break disappears. Applying the same test to rendered declarations closes that route; the name falls back to a stand-in, which is assignable to nothing it is not.
- **A spelled-out `| undefined` on an optional property is no longer a break.** `tls?: T` and `tls?: T | undefined` accept the same writes, read the same, and under `exactOptionalPropertyTypes` the second is the more permissive of the two, but the two texts differ, so the probe called it a widening. `ioredis` 5.9.2 to 5.9.3 wrote it out on two Sentinel options and failed the gate for it.
- **A class is now a base the classifier can read, and two harmless refactors stop failing the gate.** Heritage resolution answered only for interfaces, and its two callers fail in opposite directions on a base they cannot resolve: a dropped base counts as a loss, a member found on no base counts as gone. A class base hit both. Dropping `extends SomeClass` where the class declares nothing was a proven `interface-heritage-changed`, and hoisting a member onto a base class — a move that is invisible to every consumer — was a proven `property-removed`, while the same move onto a base _interface_ was already read correctly. A class's own `extends` clause is recorded for the same reason an interface's is, so the walk continues through it. What the snapshot deliberately leaves out still counts as unseen: private and protected members are not public surface, and a class carrying one is recorded as such rather than as a class with fewer members, while an index signature — which `getMembers()` does not report on a class at all — is read off the checker, because an interface extending such a class inherits it and dropping that base takes it away.
- **`required-class-property-added` is proven on a class anyone could have implemented.** It breaks only a consumer who writes the class down by hand (`const x: Cls = {…}`, `class Mine implements Cls`); `new Cls(…)` and `class Mine extends Cls` inherit the member and keep compiling. Whether such a consumer can exist is decided by the class, not by the property: a private or protected instance member makes TypeScript compare the class nominally, so an object literal carrying every public member is still rejected and there was nobody to break. That is why the oracle scored all three of this rule's firings false positives — `commander` 9.4.1 to 9.5.0 added `Help.showGlobalOptions` and does break `implements Help`, while `got` 15.0.7 to 15.1.0 and `lru-cache` 11.4.0 to 11.5.0 added one to a class carrying private fields. Grading on that separates them, and `commander` is recovered: recall goes from 79.1% to 81.4% and precision from 97.1% to 97.2%. The grade reads the _old_ side, since the consumer it claims to break is one that already exists — a class that drops its private member and gains a required property in the same release is implementable afterwards and still breaks nobody.
- **Two grades were left alone on the evidence.** `generic-constraint-changed` stays review-only because promoting it fires on a release that only loosens a constraint (`ts-pattern` 5.6.2 undoing 5.6.1), and resolving the direction instead grades nothing at all, since the constraints name package-local types. `interface-call-signature-changed` is the second, for the reason above. `required-class-property-added` was a third until the class it fires on could be told apart from the class it cannot break; that is the entry above.
...
- **A required property added to a _class_ is review-only** (`required-class-property-added`, `class-property-became-required`). It breaks only a consumer who re-implements the class structurally; `new Cls(…)` and `class Mine extends Cls` inherit the member and keep compiling. The interface counterparts stay `proven` — interfaces exist to be implemented.
...
[0.13.0]: https://github.com/kyungseopk1m/semver-checks/compare/v0.12.0...v0.13.0
