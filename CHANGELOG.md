...
- **The accuracy gate corpus grows to 109 adjacent release pairs across 22 packages**, from 75 across 17. The new packages fill surface shapes the corpus had no coverage of rather than adding pairs: `kysely` (a class-centric query builder with deep generics and subpath entries), `jose` (29 subpath entrypoints), `ioredis` (a client class with eight constructor overloads, statics, and inheritance), `dayjs` (`export =` over a namespace), and `highlight.js` (a surface declared entirely inside `declare module 'highlight.js'`). 31 of the 109 pairs now carry probe programs alongside the package's main consumer.
- **The gate's own numbers moved, and recall moved a long way down.** `--strict` now measures 93.3% precision and **37.8% recall** against 37 real breaks, where the 75-pair corpus reported 84.6% and 100%. The corpus, not the tool, is what changed: every pair with a reported finding and a clean oracle was re-examined by reading the shipped `.d.ts` diff and writing a consumer that used the changed symbol the way the package's README does. 26 pairs previously scored safe turned out to break a consumer. The old 100% was a fact about the old corpus.
- **The naive baseline now ties `--strict` on recall.** The 45-line exported-name-and-arity control scores 87.5% / 37.8% against the tool's 93.3% / 37.8%. They agree on 11 breaks, each finds three the other misses, and 20 defeat both. The type analysis still catches what a name diff cannot — a return type widening, a subpath entry losing a symbol, a parameter narrowing — but the reverse now also holds: a renamed or re-aritied export that the baseline fires on can be graded `heuristic` here and pass. Most of the 23 misses are that calibration gap rather than blindness, since `any confidence` fires on all 37 pairs and `--strict-review` fails on all 23. Two are not: on `hono` 4.12.18 → 4.12.19 and `ky` 1.14.1 → 1.14.2 the reported findings are inert and the change that actually breaks a consumer is reported nowhere.
- **Three findings previously written off as deliberate over-reports were real breaks.** `hono` 4.12.29 → 4.12.30 was documented as safe because a narrowed parameter is contravariant, and the probe that existed tested only the implementer side; a caller relaying a Node `Buffer` breaks. `jose` 6.2.4 → 6.2.5 and `kysely` 0.29.2 → 0.29.3 were the same story. Only the `zod` `version` constant remains classified as an over-report, and its note is corrected: it breaks anyone who writes the literal down, comparison included, not only someone who pinned it.
