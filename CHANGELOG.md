...
- **What still limits recall is the variance probe, not the grading.** Almost all of the 14 remaining misses are reported at `any confidence` and fail `--strict-review`; what stops them being `proven` is that `compareTypeText` returns no verdict on type texts naming package-local types, such as `ClassArray | ClassDictionary`, `P.Pattern<T> & UnknownProperties` and `core.$ZodTypeDiscriminable<Disc>`. Two of the 14 are not that: on `hono` 4.12.18 to 4.12.19 and `ky` 1.14.1 to 1.14.2 the reported findings are inert and the change that actually breaks a consumer is reported nowhere.
...
- **The naive baseline no longer keeps pace.** The 45-line exported-name-and-arity control scores 87.5% / 32.6% against the tool's 96.7% / 67.4%. They agree on 13 breaks, 16 belong to the type analysis alone, one to the baseline alone, and 13 defeat both. The previous revision of this entry recorded a tie on recall; grading is what broke it.
