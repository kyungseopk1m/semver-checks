...
- **The naive baseline no longer keeps pace.** The 45-line exported-name-and-arity control scores 87.5% / 32.6% against the tool's 97.1% / 79.1%. They agree on 14 breaks, 20 belong to the type analysis alone, none to the baseline alone, and 9 defeat both. The previous revision of this entry recorded a tie on recall; grading is what broke it, and the probe scope is what took the baseline's last exclusive catch (`ky` 1.11.0 to 1.12.0) away from it.
...
- **Two grades were left alone on the evidence.** `required-class-property-added` stays review-only: promoting it caught `commander` 9.4.1 to 9.5.0, where `implements Help` breaks, but also fired on `got` 15.0.7 to 15.1.0 and `lru-cache` 11.4.0 to 11.5.0, and both of those classes carry private fields, so no consumer can implement them structurally at all. `generic-constraint-changed` stays review-only because promoting it fires on a release that only loosens a constraint (`ts-pattern` 5.6.2 undoing 5.6.1), and resolving the direction instead grades nothing at all, since the constraints name package-local types. `interface-call-signature-changed` is the third, for the reason above.
