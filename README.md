...
**Measured, against a compiler.** The scorecard that decides which rules are `proven` uses `tsc` as its oracle, not the author's published bump — the tool exists because authors get the bump wrong, so scoring it against that bump would be circular. Each of 109 adjacent minor/patch release pairs, across 22 packages, has a consumer program compiled against both sides; the pair is a real break iff the new side produces errors the old side did not. Major-version boundaries are excluded: there the author already knows the release is breaking, so the tool's verdict carries no decision value.

On that corpus `--strict` fires on 15 of 109 pairs. 14 of those 15 are real breaks, so precision is 93.3% and no single pair emits more than 5 proven majors. **Recall is 37.8%: the corpus holds 37 real breaks and `--strict` stays quiet on 23 of them.**

Read that number, not the precision one. It is the honest state of the gate, and it is much worse than earlier revisions of this file claimed. Those read 100% recall, which was a statement about a 75-pair corpus that happened not to contain these shapes — not about the tool. Every one of the 23 was found the same way: take a pair the corpus scored safe, read what actually changed in the shipped `.d.ts`, and write a consumer that uses the changed symbol the way the package's own README does. None needed a contrived program.

Most of the gap is confidence calibration rather than blindness. `any confidence` fires on all 37 pairs, and `--strict-review` fails on every one of the 23. They arrive under `interface-method-removed`, `required-interface-method-added`, `required-property-added`, `property-type-changed`, `return-type-changed`, `type-alias-changed` and `generic-constraint-changed`, none of which is on the five-kind `proven` allow-list. Two shapes recur hard enough to name:

- **A widening that breaks readers, not writers.** An optional property gains `| null`, a union gains a member, a return type gains an alternative. Passing a value in still compiles; reading one back out into the old type does not. This accounts for the largest share, across `ioredis`, `bullmq`, `ky`, `commander`, `hono` and `got`.
- **An interface gaining or losing members.** Losing one breaks callers, gaining a required one breaks implementers, and subclassing hides both because a subclass inherits whatever was added. `property-removed` and `class-method-removed` are proven; their interface counterparts are not.

Two of the 23 are worse than a grading gap: on `hono` 4.12.18 → 4.12.19 and `ky` 1.14.1 → 1.14.2 the tool reports findings that are provably inert while saying nothing about the change that actually breaks a consumer — a required property becoming optional, and a type predicate widening. `any confidence` fires on both pairs, but for the wrong reason, so even that 100% is a per-pair coincidence there rather than detection.

The control group is [`scripts/gate/naive-baseline.mjs`](scripts/gate/naive-baseline.mjs), a 45-line exported-name-and-arity diff with no type resolution at all, kept to answer the obvious question: does the type analysis buy anything a much dumber tool does not already get? On the current corpus it scores 87.5% precision and 37.8% recall — **the same recall as `--strict`**, on a different 14 breaks. They overlap on 11. Each catches three the other misses, and 20 of the 37 defeat both.

The three the type analysis catches alone are the answer to the question, and they are the shapes a name-and-arity diff cannot reach:
...
- `hono` 4.12.29 → 4.12.30 narrowed a parameter of `WSContextInit#send`, which no name or count reveals.

Its false positives point the other way: they are removals inside `internal/` files or underscore-prefixed members, which no consumer could import in the first place. Reading declarations rather than files is what separates the public surface from the implementation.

The three the baseline catches alone are worth naming too, because they are the sharper indictment: `highlight.js` 11.8.0 → 11.9.0 and `ky` 1.11.0 → 1.12.0 / 1.12.0 → 1.13.0 all move or re-arity an exported name, so the dumb diff fires on them while the type analysis grades its own correct finding as `heuristic` and stays silent. That is the calibration gap in one sentence.
...
Three findings that *looked* like false positives were not, and all three were settled by compiling the flagged symbol rather than by reading the diff. On `jose` 6.2.4 → 6.2.5 `ResolvedKey` gained a type parameter and its `key` property became that parameter, so a resolver returning a narrower key — which is what `createRemoteJWKSet` does — narrows the result and stops compiling on write. On `kysely` 0.29.2 → 0.29.3 the mssql adapter's migration-lock methods gained required parameters. On `hono` 4.12.29 → 4.12.30 a parameter of `WSContextInit#send` narrowed; the probe that existed tested only the implementer position, which is contravariant and therefore safe, and nobody had compiled the caller — a relay passing a Node `Buffer` through breaks. Each is now probed by the corpus.
