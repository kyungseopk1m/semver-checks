...
On that corpus `--strict` fires on 30 of 111 pairs. 29 of those 30 are real breaks, so precision is 96.7%. **Recall is 67.4%: the corpus holds 43 real breaks and `--strict` stays quiet on 14 of them.**

Read the recall, not the precision. An earlier revision of this file reported 100% recall on a 75-pair corpus, and that was a fact about which shapes the corpus contained rather than about the tool: widening a corpus to 111 pairs, and re-examining every pair it had scored safe by reading the shipped `.d.ts` and writing a consumer that uses the changed symbol the way the package's own README does, took the same number to 37.8%. Grading is what recovered it from there. Nothing was suppressed to get the precision figure; the one remaining false positive is described below.

The 14 that remain are mostly not a grading gap. `any confidence` fires on all 43 pairs, so the tool sees them, but the variance probe returns no verdict on the type texts involved and a bail proves nothing in either direction. `ClassArray | ClassDictionary` in `clsx`, `P.Pattern<T> & UnknownProperties` in `ts-pattern` and `core.$ZodTypeDiscriminable<Disc>` in `zod` all send it home empty. Two of the 14 are worse than that: on `hono` 4.12.18 -> 4.12.19 and `ky` 1.14.1 -> 1.14.2 the reported findings are provably inert while the change that actually breaks a consumer is reported nowhere, so even the 100% figure is a per-pair coincidence on those two.
...
- **An interface gaining or losing members.** Losing one breaks callers, gaining a required one breaks implementers, and subclassing hides both because a subclass inherits whatever was added. The interface rules are `proven` for that reason; the class counterparts are not, because a class is instantiated and extended rather than structurally re-implemented.

The control group is [`scripts/gate/naive-baseline.mjs`](scripts/gate/naive-baseline.mjs), a 45-line exported-name-and-arity diff with no type resolution at all, kept to answer the obvious question: does the type analysis buy anything a much dumber tool does not already get? It scores 87.5% precision and 32.6% recall on the same corpus, against 96.7% and 67.4%. They overlap on 13 breaks, 16 belong to the type analysis alone, one to the baseline alone, and 13 defeat both.

The 16 are the answer to the question, and they are the shapes a name-and-arity diff cannot reach. Among them:
...
The one the baseline catches alone is `ky` 1.11.0 -> 1.12.0, where an exported name changes form. The type analysis reports it and grades its own correct finding `heuristic`, which is the calibration gap that remains after this pass.
